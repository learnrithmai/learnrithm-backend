...
function isUser(data: { id: string; email: string; password: string; createdAt: Date | null; }): data is User {
 ...
 *
 */
const verifyTokenHelper = (token: string, secret: string): Promise<JwtPayload> => {
 ...
 */
export const generateResetPasswordToken = async (user: User): Promise<string> => {
